...
export const EditAssessmentForm: React.FC<EditAssessmentFormProps> = ({
  ...
  // Initialize from assessment data when form opens
  useEffect(() => {
    if (isOpen && assessment) {
      setTitle(assessment.title || '');
      
      // Extract from content.questions if available, otherwise use legacy structure
      let questionsData: any[] = [];
      
      if (assessment.content?.questions && Array.isArray(assessment.content.questions)) {
        questionsData = assessment.content.questions;
        setInstructions(assessment.content.instructions || '');
            ...
            {questions.map((question, qIdx) => (
              <div key={qIdx} className="bg-slate-950/50 border border-slate-800 rounded-2xl p-6 space-y-5">
                ...
                {question.type === 'scramble' && (
                  <div className="space-y-3 pt-3 border-t border-slate-700">
                    <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest ml-1">📎 Question Media (Optional)</label>
                    <input
                      type="file"
                      accept="image/*,audio/*"
                      onChange={(e) => {
                    ...
                    {question.questionMedia && (
                      <div className="space-y-2">
                        ...
                        {question.questionMedia.type?.startsWith('image') ? (
                          <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 w-fit">
                            <img src={question.questionMedia.preview} alt="Scramble preview" className="h-32 w-auto rounded object-cover" />
                            <p className="text-[8px] text-slate-400 mt-2 font-semibold">{question.questionMedia.name}</p>
                          </div>
                        ) : (
                          <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 space-y-2 w-fit">
